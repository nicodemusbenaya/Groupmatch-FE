...
export const RoomProvider = ({ children }) => {
  ...
  ========================= */
  const startMatchmaking = async () => {
    if (!user || searchingRef.current) return;
    ...
    try {
      ...
      if (res.data?.room_id) {
        const roomRes = await api.get(`/rooms/${res.data.room_id}`);
        handleMatchFound(res.data.room_id, roomRes.data);
        return;
      }

      toast({
        title: "Masuk Antrian",
        description: "Mencari tim...",
      });
      ...
      toast({
        title: "Gagal",
        description: err.response?.data?.detail || "Gagal matchmaking",
        variant: "destructive",
      });
    }
  };
    ...
    pollingRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (status?.status === "matched" && status.room_id) {
          ...
          const roomRes = await api.get(`/rooms/${status.room_id}`);
          handleMatchFound(status.room_id, roomRes.data);
        }
      } catch (e) {
        console.log("Polling error:", e?.response?.status);
      }
    }, 2000);
  };
  ...
  const handleMatchFound = (roomId, roomData) => {
    if (activeRoom) return; // ⛔ prevent double
    ...
    toast({ title: "Tim Terbentuk!", description: "Room berhasil dibuat" });
  };
